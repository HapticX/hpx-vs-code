...
var
  nimSymbolProvider* {.exportc.} = block:
    var o = newJsObject()
    o.provideWorkspaceSymbols = provideWorkspaceSymbols
    o.provideDocumentSymbols = provideDocumentSymbols
    o
  nimDocSymbolProvider* {.exportc.} = nimSymbolProvider.to(VscodeDocumentSymbolProvider)
  nimWsSymbolProvider* {.exportc.} = nimSymbolProvider.to(VscodeWorkspaceSymbolProvider)
